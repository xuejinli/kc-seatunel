...
public class TimeStampMilliTZConverter implements Converter<TimeStampMilliTZVector> {
    @Override
    public Object convert(int rowIndex, TimeStampMilliTZVector fieldVector) {
        return fieldVector.isNull(rowIndex) ? null : fieldVector.getObject(rowIndex);
    }

    @Override
    public boolean support(Types.MinorType type) {
        return Types.MinorType.TIMESTAMPMILLITZ == type;
    }
}
