...
public class MapConverter implements Converter<MapVector> {
    ...
    @Override
    public Object convert(int rowIndex, MapVector fieldVector, List<Function> genericsConvert) {
        UnionMapReader reader = fieldVector.getReader();
        reader.setPosition(rowIndex);
        Map<Object, Object> mapValue = new HashMap<>();
        while (reader.next()) {
            Object key = genericsConvert.get(0).apply(processTimeZone(reader.key().readObject()));
            Object value =
                    genericsConvert.get(1).apply(processTimeZone(reader.value().readObject()));
